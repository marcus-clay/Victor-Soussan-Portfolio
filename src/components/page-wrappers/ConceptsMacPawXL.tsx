    ...
    <div style={{ fontFamily: font }}>
      ...
      <div className="sticky top-14 z-30 bg-white/90 backdrop-blur-xl border-b border-gray-100 py-2">
        <div className="max-w-[1200px] mx-auto px-6 flex items-center gap-4 overflow-x-auto scrollbar-hide">
          {sections.map((s, i) => <button key={i} onClick={() => document.getElementById(`mp-${i}`)?.scrollIntoView({ behavior: 'smooth' })} className="text-[19px] font-semibold text-gray-400 hover:text-gray-700 whitespace-nowrap transition-colors">{s.label}</button>)}
        </div>
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-[180px_1fr] gap-12">
        <div className="hidden lg:block">
          <div className="sticky top-24">
            ...
            {sections.map((s, i) => <button key={i} onClick={() => document.getElementById(`mp-${i}`)?.scrollIntoView({ behavior: 'smooth' })} className="block w-full text-left py-1.5 text-[20px] font-medium text-gray-400 hover:text-gray-700 transition-colors">{s.label}</button>)}
