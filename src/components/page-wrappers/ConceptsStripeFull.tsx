...
function Expand({ summary, children }: { summary: string; children: React.ReactNode }) {
  ...
  return (
    <div>
      {!open && <p className="text-[16px] text-gray-500 leading-relaxed">{summary}</p>}
      ...
      <button onClick={() => setOpen(!open)} className="mt-2 text-[14px] font-medium flex items-center gap-1 transition-colors hover:text-blue-700" style={{ color: blue, fontFamily: font }}>
      ...
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-4 py-5 text-left group">
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-3">
            <h3 className="text-[16px] font-semibold group-hover:text-blue-600 transition-colors">{title}</h3>
...
function Vid({ src, label, onClick }: { src: string; label: string; onClick?: () => void }) {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden bg-gray-100 cursor-pointer"
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"><p className="text-[12px] text-white font-medium">{label}</p></div>
    </div>
  );
}
        ...
        <div className="ml-auto hidden md:flex items-center gap-0.5">
          {([['home','Home'],['work','Work'],['about','About'],['blog','Resources'],['gallery','Interfaces']] as [PageId,string][]).map(([id,label]) => (
            <motion.button key={id} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={spB} onClick={() => go(id)}
              className={`px-3 py-1.5 text-[14px] font-semibold transition-all ${page === id ? 'text-gray-900 border-b-2 border-gray-900 rounded-none' : 'text-gray-400 hover:text-gray-900'}`}>
              {label}
            </motion.button>
          ))}
...
function FloatCTA() {
  ...
  return <AnimatePresence>{show && (
    <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} transition={spB}
      ...
      <span className="text-[13px] text-gray-500 font-medium hidden sm:block">Available for new projects</span>
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-4 py-2 text-white rounded-xl text-[13px] font-semibold" style={{ background: blue }}>Book a call</motion.button>
    </motion.div>
  )}</AnimatePresence>;
}
...
function Surprise({ go }: { go: (p: PageId) => void }) {
  ...
  return <AnimatePresence>{show && (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={spB}
      ...
      <p className="text-[14px] text-gray-700 font-medium leading-snug pr-4">I wrote a 9-chapter guide on Claude Code for designers.</p>
      <button onClick={() => { go('blog'); setDismissed(true); }} className="mt-2 text-[13px] font-semibold flex items-center gap-1" style={{ color: blue }}>Read the guide <ArrowRight size={12} /></button>
    </motion.div>
  )}</AnimatePresence>;
}
  ...
  return (
    <section className="py-16 px-6 border-t border-gray-100 bg-gray-50/30">
      <div className="max-w-[1200px] mx-auto">
        <p className="text-[12px] font-bold text-gray-400 uppercase tracking-[0.12em] mb-6">Continue exploring</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {items.map((item, i) => (
            <motion.button key={i} onClick={item.onClick} whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }} transition={spB}
              ...
              <p className="text-[15px] font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{item.label}</p>
  ...
  return (
    <div style={{ fontFamily: font }}>
      ...
      <section className="py-28 md:py-36 px-6 relative overflow-hidden">
        <div className="absolute inset-0 -z-10" style={{ background: 'linear-gradient(160deg, #f6f9fc 0%, #eef2ff 50%, #faf5ff 100%)', clipPath: 'polygon(0 0, 100% 0, 100% 85%, 0 100%)' }} />
        <div className="max-w-[800px] mx-auto">
          ...
          <F delay={0.1}>
            <Expand summary={T.hero.desc.slice(0, 180) + '.'}>
              <p className="text-[16px] text-gray-500 leading-relaxed mb-3">{T.hero.desc}</p>
              <p className="text-[14px] text-gray-400 tracking-wide">{T.hero.positioning}</p>
            </Expand>
          </F>
          <F delay={0.15}>
            <div className="flex gap-3 flex-wrap mt-8">
              <motion.button whileHover={{ scale: 1.03, y: -1 }} whileTap={{ scale: 0.97 }} transition={spB} onClick={() => go('work')}
                className="group px-7 py-3.5 text-white rounded-lg text-[15px] font-semibold flex items-center gap-2" style={{ background: blue }}>
              ...
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={spB} onClick={() => go('about')}
                className="px-7 py-3.5 rounded-lg text-[15px] text-gray-500 border border-gray-200 hover:border-gray-300 font-medium">About me</motion.button>
            </div>
          </F>
        </div>
      </section>
      ...
      <F><div className="max-w-[1400px] mx-auto px-4 mb-24">
        <motion.div whileHover={{ y: -3 }} transition={spB} onClick={() => go('case')}
          className="group cursor-pointer rounded-xl overflow-hidden bg-white border border-gray-200/60 hover:border-gray-300 hover:shadow-lg hover:shadow-gray-900/[0.04] transition-all">
          ...
          <div className="p-8">
            ...
            <Expand summary={PROJECTS[0].summary.slice(0, 100) + '.'}>
              <p className="text-[16px] text-gray-500 leading-relaxed">{PROJECTS[0].summary}</p>
              <div className="flex flex-wrap gap-2 mt-3">{PROJECTS[0].deliverables.map((d: string) => <span key={d} className="text-[12px] px-2.5 py-1 rounded-lg bg-gray-50 text-gray-500 border border-gray-100">{d}</span>)}</div>
            </Expand>
          </div>
        </motion.div>
      </div></F>
            ...
            {PROJECTS.slice(1, 5).map((p, i) => (
              <F key={p.id} delay={i * 0.06}>
                <motion.div whileHover={{ y: -3 }} transition={spB} onClick={() => go('case')}
                  ...
                  <div className="p-5">
                    <h3 className="text-[17px] font-semibold group-hover:text-blue-600 transition-colors">{p.title}</h3>
              ...
              <F key={i} delay={i * 0.08}>
                <div className="p-7 rounded-2xl bg-white border border-gray-100">
                  ...
                  <Expand summary={p.items[0]}>
                    <ul className="space-y-2">{p.items.map((item: string, j: number) => <li key={j} className="text-[15px] text-gray-500 leading-relaxed flex gap-2"><span className="text-gray-300 shrink-0">·</span>{item}</li>)}</ul>
                  </Expand>
                </div>
              </F>
            ))}
        ...
        <div className="max-w-[1200px] mx-auto">
              ...
              <F key={i} delay={i * 0.08}>
                <div className="min-w-[360px] max-w-[440px] p-6 rounded-2xl bg-white border border-gray-100 snap-start shrink-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-[13px] font-semibold text-gray-900">{t.author}</p><p className="text-[12px] text-gray-400">{t.role}</p></div>
                  </div>
                </div>
              </F>
            ))}
          ...
          <F>
            <div className="p-6 rounded-2xl bg-white border border-gray-100">
              <p className="text-[12px] font-bold text-gray-400 uppercase tracking-[0.12em] mb-4">Companies I have worked with</p>
              <div className="flex flex-wrap gap-x-8 gap-y-3">{LOGOS.map(n => <span key={n} className="text-[16px] text-gray-300 font-semibold hover:text-gray-500 transition-colors cursor-default">{n}</span>)}</div>
            </div>
          </F>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <F delay={0.05}>
            <motion.div whileHover={{ y: -2 }} transition={spB} onClick={() => go('blog')}
              ...
              <div className="flex items-center gap-2 mb-3"><BookOpen size={14} style={{ color: blue }} /><span className="text-[12px] font-bold uppercase tracking-wider" style={{ color: blue }}>Guide · 9 chapters</span></div>
              <h3 className="text-[18px] font-semibold group-hover:text-blue-600 transition-colors">Getting started with Claude Code</h3>
              <p className="text-[14px] text-gray-500 mt-1">Complete guide for designers: installation to deployment, visual quality, skills, and Figma MCP.</p>
            </motion.div>
          </F>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {ARTICLES.slice(0, 3).map((a, i) => (
              <F key={a.id} delay={0.08 + i * 0.04}>
                <ExpandItem title={a.title_en} subtitle={`${a.category} · ${a.date}`}>
                  <p className="text-[15px] text-gray-500 leading-relaxed">{a.body_en.slice(0, 200)}...</p>
                  <button onClick={() => go('article')} className="mt-2 text-[14px] font-medium flex items-center gap-1" style={{ color: blue }}>Read full article <ArrowRight size={12} /></button>
                </ExpandItem>
              </F>
            ))}
          </div>
        ...
        <F>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={spB}
              className="px-8 py-4 text-white rounded-lg text-[16px] font-semibold flex items-center justify-center gap-2" style={{ background: blue }}>Book a call <Calendar size={16} /></motion.button>
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={spB}
              className="px-8 py-4 rounded-lg text-[16px] text-gray-500 border border-gray-200 flex items-center justify-center gap-2">Email <Envelope size={16} /></motion.button>
          </div>
        </F>
      </section>
            ...
            <ExpandItem title={p.title} subtitle={`${p.role} · ${p.period}`} badge={p.format === 'case-study' ? 'Case Study' : 'Short'}>
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  <p className="text-[16px] text-gray-500 leading-relaxed mb-3">{p.summary}</p>
                  <div className="flex flex-wrap gap-2 mb-3">{p.deliverables.slice(0, 4).map((d: string) => <span key={d} className="text-[12px] px-2.5 py-1 rounded-lg bg-gray-50 text-gray-500 border border-gray-100">{d}</span>)}</div>
                  <button onClick={() => go('case')} className="text-[14px] font-medium flex items-center gap-1" style={{ color: blue }}>View case study <ArrowRight size={12} /></button>
                </div>
              </div>
            </ExpandItem>
          </F>
    ...
    <div style={{ fontFamily: font }}>
      ...
      <section className="pt-16 pb-10 px-6">
        <div className="max-w-[800px] mx-auto">
          ...
          <F delay={0.03}>
            <div className="flex items-center gap-6 text-[14px] text-gray-500 mb-4"><span className="font-semibold text-gray-900">Toolkit</span><span>·</span><span>{TK.meta.type}</span><span>·</span><span>{TK.meta.period}</span></div>
            ...
            <Expand summary={TK.hero.description.slice(0, 160) + '.'}>
              <p className="text-[16px] text-gray-500 leading-relaxed">{TK.hero.description}</p>
            </Expand>
          </F>
        </div>
      </section>
      ...
      <F><div className="max-w-[1400px] mx-auto px-4 mb-16">
        ...
        <div className="max-w-[800px] mx-auto mt-4"><p className="text-[13px] font-semibold text-gray-900">Toolkit: Batch editing</p><p className="text-[12px] text-gray-400">Managing multiple tasks across construction sites simultaneously.</p></div>
      </div></F>
        ...
        <div className="hidden lg:block">
          <div className="sticky top-24">
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              ...
              <motion.button whileHover={{ x: 2 }} transition={spB} onClick={() => go('blog')} className="text-[13px] font-semibold text-gray-500 hover:text-blue-600 transition-colors block">Claude Code Guide</motion.button>
            </div>
          </div>
        </div>
        <div>
              ...
              <F>
                ...
                {s.extra ? (
                  <Expand summary={s.text.slice(0, 160) + '.'}>
                    <p className="text-[17px] text-gray-600 leading-[1.75] mb-4">{s.text}</p>
                    {s.extra && <p className="text-[17px] text-gray-600 leading-[1.75]">{s.extra}</p>}
                    {s.role && <p className="text-[15px] text-gray-500 mt-4 p-4 rounded-xl bg-gray-50 border border-gray-100"><span className="font-semibold text-gray-700">My role: </span>{s.role}</p>}
                  </Expand>
                ) : (
                  <p className="text-[17px] text-gray-600 leading-[1.75]">{s.text}</p>
                )}
              </F>
              {i < 5 && UI.toolkit[i * 2] && (
                <F><div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {UI.toolkit.slice(i * 2, i * 2 + 2).filter(Boolean).map((img, j) => (
                    <div key={j} className="group rounded-2xl overflow-hidden bg-gray-50 cursor-pointer" onClick={() => setLbIdx(3 + i * 2 + j)}>
                      ...
                      <div className="p-3"><p className="text-[13px] font-semibold text-gray-900">{s.label} screen {j + 1}</p></div>
                    </div>
                  ))}
                </div></F>
              )}
              {i === 6 && (
                <F><div className="mt-8 grid grid-cols-3 gap-4">
                  {[{ v: TK.impact.customers, d: TK.impact.customersDesc }, { v: TK.impact.seriesA, d: TK.impact.seriesADesc }, { v: TK.impact.enterprise, d: TK.impact.enterpriseDesc }].map((m, mi) => (
                    <div key={mi} className="p-5 rounded-2xl bg-gray-50 border border-gray-100"><span className="text-2xl font-bold" style={{ color: blue }}>{m.v}</span><p className="text-[14px] text-gray-500 mt-1">{m.d}</p></div>
                  ))}
                </div></F>
              )}
            </section>
      ...
      <section className="py-14 px-6"><div className="max-w-[800px] mx-auto"><F>
        <div className="p-8 rounded-2xl bg-gray-50 border border-gray-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-[14px] font-semibold">{TK.testimonial.author}</p><p className="text-[12px] text-gray-400">{TK.testimonial.role}</p></div>
          </div>
        </div>
      </F></div></section>
          ...
          <div className="md:col-span-7">
            ...
            <F delay={0.05}>
              <Expand summary={T.bio.p1}>
                <p className="text-[17px] text-gray-600 leading-[1.75] mb-4">{T.bio.p1}</p>
                <p className="text-[17px] text-gray-600 leading-[1.75] mb-4">{T.bio.p2}</p>
                <div className="p-4 rounded-xl bg-gray-50 border border-gray-100 mt-4">
                  <h4 className="text-[14px] font-semibold mb-2">Core strengths</h4>
                  <ul className="space-y-1">{T.bio.bullets.map((b: string, i: number) => <li key={i} className="text-[15px] text-gray-500 flex gap-2"><span className="text-gray-300">·</span>{b}</li>)}</ul>
                </div>
              </Expand>
            </F>
            <F delay={0.1}>
              ...
              {career.map((c, i) => (
                <ExpandItem key={i} title={c.co} subtitle={c.role} badge={c.y}>
                  ...
                  <p className="text-[15px] text-gray-600 leading-relaxed">{c.expanded}</p>
                </ExpandItem>
              ))}
            </F>
            <F delay={0.15}>
          ...
          <div className="md:col-span-5">
            <F delay={0.2}><div className="md:sticky md:top-20 space-y-5">
              ...
              <div className="p-5 rounded-2xl bg-white border border-gray-100">
                ...
                <div className="flex flex-wrap gap-2">{['Figma', 'Claude Code', 'VS Code', 'Next.js', 'Tailwind', 'Vercel', 'Linear', 'Notion'].map(t => <span key={t} className="text-[13px] px-3 py-1.5 rounded-xl bg-gray-50 text-gray-600 border border-gray-100">{t}</span>)}</div>
              </div>
              <div className="p-5 rounded-2xl bg-white border border-gray-100">
  ...
  return (
    <div style={{ fontFamily: font }}>
      <section className="pt-24 pb-20 px-6"><div className="max-w-[1200px] mx-auto">
        ...
        <F delay={0.05}><p className="text-[18px] text-gray-500 mb-12">{T.signals.subtitle}</p></F>
        <F delay={0.08}>
          <div className="p-6 rounded-2xl bg-white border border-gray-100 mb-8 cursor-pointer group" onClick={() => go('article')}>
            ...
            <Expand summary="Complete guide for designers: from installation to deployment.">
              <p className="text-[16px] text-gray-500 leading-relaxed">A comprehensive walkthrough covering installation, configuration, visual quality tips, skills system, Figma MCP integration, and deployment workflows. Written from 6 months of daily practice.</p>
            </Expand>
          </div>
        </F>
        ...
        {ARTICLES.map((a, i) => (
          <F key={a.id} delay={0.12 + i * 0.03}>
            <ExpandItem title={a.title_en} subtitle={`${a.category} · ${a.date}`}>
              <p className="text-[15px] text-gray-500 leading-relaxed mb-3">{a.body_en.slice(0, 250)}...</p>
              <button onClick={() => go('article')} className="text-[14px] font-medium flex items-center gap-1" style={{ color: blue }}>Read full article <ArrowRight size={12} /></button>
            </ExpandItem>
          </F>
        ))}
      </div></section>
  ...
  return (
    <div style={{ fontFamily: font }}>
      <section className="pt-24 pb-8 px-6"><div className="max-w-[680px] mx-auto">
        ...
        <F delay={0.03}>
          <span className="px-3 py-1 rounded-xl bg-gray-100 text-[12px] font-semibold text-gray-600">{a.category}</span>
      ...
      <section className="py-16 px-6 bg-gray-50/50"><div className="max-w-[800px] mx-auto">
        ...
        {ARTICLES.slice(1, 4).map((ra, i) => (
          <ExpandItem key={ra.id} title={ra.title_en} subtitle={`${ra.category} · ${ra.date}`}>
            <p className="text-[15px] text-gray-500 leading-relaxed">{ra.body_en.slice(0, 150)}...</p>
          </ExpandItem>
        ))}
      </div></section>
    </div>
        ...
        <F delay={0.08}><div className="flex gap-2 mb-8">
          {['all', ...groups.map(g => g.id)].map(id => (
            <motion.button key={id} whileTap={{ scale: 0.95 }} onClick={() => setFilter(id)}
              className={`px-4 py-2 rounded-xl text-[13px] font-medium transition-all ${filter === id ? 'text-white' : 'bg-gray-50 text-gray-500 hover:text-gray-700 border border-gray-100'}`}
          ...
          {allItems.map((item, i) => (
            <F key={`${filter}-${i}`} delay={Math.min(i * 0.02, 0.15)}>
              <div className="break-inside-avoid group cursor-pointer" onClick={() => setLbIdx(i)}>
                ...
                <div className="mt-2 px-1"><p className="text-[13px] font-semibold text-gray-900">{item.project}: {item.screen}</p></div>
              </div>
            </F>
          ))}
        </div>
...
/* ═══ EXPORT ═══ */
export function StripeFull() {
  const [page, setPage] = useState<PageId>('home');
  const go = (p: PageId) => { setPage(p); window.scrollTo({ top: 0, behavior: 'instant' as ScrollBehavior }); };
  return (
    <div className="min-h-screen bg-white text-gray-900" style={{ fontFamily: "'Public Sans', system-ui, sans-serif" }}>
      <Nav page={page} go={go} />
